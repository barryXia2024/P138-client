 ...
 * 投注类型中文字典
 */
export const BetTypeDict: Record<BetType, string> = {
  single: '单式',
  multiple: '复式',
  danTuo: '胆拖',
};
...
export const useBetSlipStore = create<BetSlipState>()(
  persist(
    set => ({
      ...
      selectedPassTypes: [],
      // 当前选中的投注索引
      selectedBetIndex: undefined,

      // 批量添加投注
      addBets: newBets =>
        set(state => ({
          bets: [...state.bets, ...newBets.map(bet => ({
            ...bet,
            timestamp: Date.now()
          }))],
        })),
      ...
      removeBet: index =>
        set(state => ({
          ...
          selectedBetIndex: state.selectedBetIndex === index ? undefined : state.selectedBetIndex
        })),
      ...
      // 清空所有投注
      clearBets: () => set({ bets: [], selectedBetIndex: undefined }),

      // 设置当前选中的投注索引
      setSelectedBetIndex: (index?: number) => set({ selectedBetIndex: index }),
    }),
    {
      name: 'betSlipStore',
    }
