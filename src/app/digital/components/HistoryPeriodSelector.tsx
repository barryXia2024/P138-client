...
interface HistoryPeriodSelectorProps {
  lotteryData?: LotteryDataSource.CharityLotteryDataSource&{currentTermNo: string,defaultTermNo: string};
  onPeriodChange?: (
    lotteryData: LotteryDataSource.CharityLotteryDataSource&{currentTermNo: string,defaultTermNo: string},
  ) => void;
  needZero?: boolean;
                ...
                onPeriodChange?.({
                  ...
                  currentTermNo:
                    lotteryData?.currentTermNo == item.termNo
                      ...
                      : item.termNo,
                  defaultTermNo: lotteryData.defaultTermNo ?? lotteryData?.currentTermNo,
                });
              } else {
                Toast.show('当前数据不存在');
              }
    ...
    <View
      ...
      <TouchableOpacity
        ...
        <View>
          <Text className="text-md font-bold">
            {lotteryData?.currentTermNo}期
          </Text>
        </View>
        <View className="text-md mr-4 flex-row gap-3">
          <Text> 截止时间: </Text>
          {lotteryData?.buyEndTime?.split(' ').map((item, index) => {
            return (
              <Text
                key={item}
      ...
      {isExpanded && (
        <ScrollView
          ...
          {lotteryData?.vos?.map(renderHistoryItem)}
        </ScrollView>
      )}
    </View>
  );
