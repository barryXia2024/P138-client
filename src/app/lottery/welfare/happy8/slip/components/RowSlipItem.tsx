...
const RowSlipItem = ({
  ...
  const playType = () => {
    if (item.playMode === Happy8PlayEnum.normal) {
      return `${item.subPlayMode}[${Number(item.betCount) === 1 ? '单式' : '复式'}]`;
    } else {
      return `${item.subPlayMode}[胆拖]`;
    }
  };

  const DantuoItem = () => {
    if (item.playMode === Happy8PlayEnum.dantuo) {
      return (
