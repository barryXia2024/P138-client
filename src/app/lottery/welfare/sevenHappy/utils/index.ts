...
export function resolveSevenHappyStrategy(
  rules: PositionRule[],
  opts?: {play?: SevenHappyPlayEnum; subPlay?: SevenHappyPlayEnum},
) {
  // 直选 · 组合胆拖
  if (opts?.play === SevenHappyPlayEnum.normal) {
    return {
      quickPick(n: number): number[][] {
        
        const balls = pickDistinct(n, 30);
      ...
      getBetCount(positions: number[][]): number {
        ...
        const redCombinations = combination(redBalls.length, 7);
        console.log(redCombinations,'========redCombinations')

        return redCombinations;
      },
    } as const;
  }

  if (opts?.play === SevenHappyPlayEnum.dantuo) {
    return {
      quickPick(n: number): number[][] {
        // 胆拖模式：前区胆码1-5个，拖码2-28个，后区直接选蓝球
        const frontDan = pickDistinct(6, 30); // 前区胆码2个
        const frontTuo = pickDistinct(2, 30, frontDan); // 前区拖码4个（排除胆码）

        return [frontDan, frontTuo];
      },
      getBetCount(positions: number[][]): number {
        ...
        const blueCombinations = frontTuo.length;

        return frontCombinations * frontTuo.length;
      },
    } as const;
  }
  ...
  return {
    quickPick(n: number): number[][] {
      return rules.map(rule =>
        pickDistinct(n, rule.numberRange),
      );
    },
    getBetCount(positions: number[][]): number {
      // 对于双色球，默认也使用组合数计算
