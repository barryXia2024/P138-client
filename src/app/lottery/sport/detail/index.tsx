...
import StreamPlayer from 'src/modules/live/player/stream';
...
import DetailInfos from 'src/modules/lottery/sport/detail/detailInfos';
import {useSportDetail} from './hooks';
import {useAnchorStore} from 'src/app/live/store/anchorStore';
import {useLocalSearchParams} from 'expo-router';

const SportDetail = () => {
  const {state, startAnchorLive, watchLive, exitLive} = useSportDetail();
    ...
    <View style={styles.flex1}>
      ...
      {isLiveStarted && (
        <View style={[styles.playerContainer, {height: (kScreenHeight * 2) / 5}]}>
          {isAnchor ? (
            // 主播播放器
            <StreamPlayer liveUrl={state.live.liveUrl!} onExit={exitLive} />
          ) : (
            // 观众播放器
      ...
      {!isLiveStarted && (
        <DetailBaseInfo
          ...
          onChange={isAnchor ? startAnchorLive : watchLive}
        />
      )}
