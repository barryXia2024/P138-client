...
import {useBettingListStore as useBettingListStoreArrangedThree} from '../sport/digital/arrangedThree/slip/store/useBettingListStore';
import {useBettingListStore as useBettingListStoreArrangedFive} from '../sport/digital/arrangedFive/slip/store/useBettingListStore';
import {useBettingListStore as useBettingListStoreSevenHappy} from '../welfare/sevenHappy/slip/store/useBettingListStore';
import {useBettingListStore as useBettingListStoreHappy8} from '../welfare/happy8/slip/store/useBettingListStore';
import {useSuperLottoStore} from '../digital/superLotto/store';
import {useBettingListStore} from '../welfare/doubleBall/slip/store/useBettingListStore';

const PayConfirm = () => {
  const {dictString} = useLocalSearchParams();
  const {clearAll: clearAllArrangedThree} = useBettingListStoreArrangedThree();
  const {clearAll: clearAllSevenHappy} = useBettingListStoreSevenHappy();
  const {clearAll: clearAllHappy8} = useBettingListStoreHappy8();
  const {clearSelection} = useSuperLottoStore();
  const {clearAll: clearAllDoubleBall} = useBettingListStore();
  const creatOrderCommand = JSON.parse(
    dictString as string,
  ) as ServerCoreOrder.CreateOrderCommand;
  console.log(creatOrderCommand, 'creatOrderCommand');
  ...
  const {clearAll: clearAllArrangedFive} = useBettingListStoreArrangedFive();
  ...
  const commmit = () => {
    ...
    createLotteryOrder(creatOrderCommand).then(res => {
      ...
      if (res.success) {
        clearAllArrangedThree();
        clearAllArrangedFive();
        clearAllSevenHappy();
        clearAllHappy8();
        clearSelection();
        clearAllDoubleBall();
