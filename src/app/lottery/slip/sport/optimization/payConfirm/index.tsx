...
const PayConfirm = () => {
  ...
  const {creatOptimizationOrder} = useCreateOrder();

  const jcBetsInfo = JSON.parse(jcBetsInfoString) as ServerCoreOrder.CreateOrderCommand;
  ...
  const commmit = () => {
    const orderAmount = jcBetsInfo.betAmount;
    // const ix
    const amount =
      lotteryInfo?.lotteryType === SportLotteryTypeEnum.Sport
        ? jcBetsInfo.betAmount  
        : orderAmount;
    if (
      ...
      <View style={styles.content}>
        ...
        <View style={styles.row}>
          ...
          <Text
            ...
            {formatCurrency(jcBetsInfo.betAmount)}
          </Text>
        </View>
      </View>
