...
const digitalMultiplierArr = ['10', '20', '50', '99'];
...
const CustomerMultiplier: React.FC<{
  ...
  const config = {
    digital: {
      multiplierArr: digitalMultiplierArr,
  ...
  const handleMultiplierChange = (m: string) => {
    if (m === '删除') {
      ...
      console.log('maxMultiplierTitle', tempMultiplier);
      if(maxMultiplierTitle==='期'&&Number(tempMultiplier+m)>99){
        setTempMultiplier('99');
        config[type].maxMultiplier = '99';
        Toast.show('最多投99期' );
        return;
      }
      ...
      <View style={styles.container}>
        ...
        <View style={styles.quickMultiplierContainer}>
          {config[type].multiplierArr.map(item => (
            <TouchableOpacity
              key={item}
              onPress={() => setTempMultiplier(item)}
              style={styles.quickMultiplierButton}>
        ...
        <View style={styles.keyboardContainer}>
          {config[type].keybordArr.map(item => (
            <TouchableOpacity
              key={item}
              onPress={() => handleMultiplierChange(item)}
