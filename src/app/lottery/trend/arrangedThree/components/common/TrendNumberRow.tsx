...
const colors = ['blue','red','green','#7b3cc1'];
      ...
      {numbers?.map((item, index) => {
        ...
        return (
          <View key={`num-${index}`} style={local.cell}>
            {isWinning ? (
                <View style={[local.winningBall, isBlue && local.blueBall, {backgroundColor: colors[Math.floor(index / 10) % colors.length]}]}>
                <Text style={local.winningText}>{item.num}</Text>
              </View>
            ) : (
