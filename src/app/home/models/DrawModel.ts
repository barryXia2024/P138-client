...
import { GetDrawStatisticsParams, GetDrawAnnouncementParams, DrawData } from '../types';
...
export class DrawModel {
   ...
   * @returns 开奖统计响应
   */
  static async getDrawStatistics(params: GetDrawStatisticsParams) {
    try {
   ...
   * @returns 开奖公告响应
   */
  static async getDrawAnnouncement(params: GetDrawAnnouncementParams) {
    try {
   ...
   * @returns 足球开奖数据
   */
  static async getFootballDrawData(): Promise<LotteryDrawAnnouncement.SportsDrawAnnouncement[] | null> {
    try {
      // 获取统计信息
      const statisticsRes = await this.getDrawStatistics({
        lotteryName: 'FootballLottery' as CommonCommonEnum.LotteryName,
      });

      if (!statisticsRes.success || !statisticsRes.data || statisticsRes.data.length === 0) {
        return null;
      }
   ...
   * @returns 篮球开奖数据
   */
  static async getBasketballDrawData(): Promise<LotteryDrawAnnouncement.SportsDrawAnnouncement[] | null> {
    try {
      // 获取统计信息
      const statisticsRes = await this.getDrawStatistics({
        lotteryName: 'BasketballLottery' as CommonCommonEnum.LotteryName,
      });
   ...
   * @returns 所有开奖数据
   */
  static async getAllDrawData(): Promise<DrawData> {
    try {
      const [footballData, basketballData] = await Promise.all([
   ...
   * @returns 是否有效
   */
  static isValidDrawData(drawData: LotteryDrawAnnouncement.SportsDrawAnnouncement[] | null): boolean {
    return drawData !== null && 
           drawData.length > 0 && 
           drawData[0] !== undefined;
  }
   ...
   * @returns 显示文本
   */
  static getDrawDisplayText(drawData: LotteryDrawAnnouncement.SportsDrawAnnouncement[] | null): string {
