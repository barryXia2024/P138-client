...
import { useSportsCompetitionFilterStore } from 'src/modules/lottery/store';
...
const MAX_HEIGHT = SCREEN_HEIGHT * 0.7;
 ...
 const FilterDialog = ({
  ...
  const availableLeagues = useMemo(() => {
    if (!filterDialog.matchData?.length) return [];

    const leagueSet = new Set<string>();
    filterDialog.matchData?.forEach(match => {
  ...
  useEffect(() => {
    if (filterDialog.isVisible) {
      ...
      // 只在足球模式下设置这些选项
      if (filterDialog.lotteryName === 'FootballLottery') {
        setTempOddsRanges(selectedOddsRanges);
        setTempOnlyWinDrawLose(onlyWinDrawLose);
      }
    }
  }, [filterDialog.isVisible, availableLeagues, selectedLeagues, isFirstOpen, filterDialog.lotteryName]);
  ...
  const tempSelectTopLeagues = () => {
    if (filterDialog.lotteryName === 'BasketballLottery') {
      // 篮球选择 NBA 和 CBA
  ...
  return (
    <CustomModal
      ...
      isVisible={filterDialog.isVisible}>
      <View className='bg-white w-full '  >
        <ScrollView
          ...
          <View className='bg-white p-4'>
              ...
              <XStack space='$4'>
                ...
                <TouchableOpacity
                  ...
                  <Text style={{ color: '#000' }}>
                    {filterDialog.lotteryName === 'BasketballLottery' ? 'NBA/CBA' : '五大联赛'}
                  </Text>
                </TouchableOpacity>
              </XStack>
            ...
            {/* 只在足球模式下显示赔率筛选和胜平负选项 */}
            {filterDialog.lotteryName === 'FootballLottery' && (
              <View>
                {/* 赔率筛选 */}
