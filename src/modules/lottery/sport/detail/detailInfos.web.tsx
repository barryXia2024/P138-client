...
import  { FollowOrderList } from 'src/app/follow';
const statTabs = [
  {label: '赛况', key: 0},
  {label: '聊天', key: 1},
  {label: '指数', key: 2},
        ...
        renderItem={({index}) => (
          <View style={{flex: 1, height: '100%'}}>
            {index === 0 &&
              (competitionType === 'LQ' ? (
                <BasketballCompetitionStatistics
                  liveInfo={liveInfo as ServerCommonLive.BasketBallCompetition}
                />
              ) : (
                <CompetitionStatistics competitionType={competitionType} />
              ))}
            {index === 1 && (
